"""
EXP011  —  ATR-based minimum SL distance on 15m

Hypothesis:
    The current SL placement uses the trigger candle's natural high/low on 15m.
    In ranging / low-volatility markets (e.g. ETH/USDT Apr 23–24 2026),
    this produces SL distances as small as 0.19%–0.44% of price — smaller than
    the normal bar-to-bar noise of the asset.  The result is systematic SL hits
    before price has room to move toward TP (2× risk).

    Observed in live paper trading:
      - 6 consecutive SL hits in ETH/USDT shorts (Apr 23 21:00 → Apr 24 03:00)
      - All exits within 1–5 USD of entry despite 2:1 R:R target
      - Equity 10200 → 9694 in 3 days

    Fix: enforce a *minimum* SL distance of 1.0× the 14-period ATR on 15m.
    If the candle extreme is already further than ATR × multiplier, keep it as-is
    (we never tighten the SL below the candle structure). If it is closer, expand
    it to ATR × multiplier.  TP is always recomputed at 2× the actual risk used.

    This keeps the 2:1 R:R intact while ensuring the SL is at least as wide
    as recent market noise.

Baseline    : EXP009 + EXP010 (current production stack)
  BTC longs : 99 trades | WR=39.4% | PF=1.297 | Return=+18.45% | MaxDD=7.78%
  ETH both  : 207 trades | WR=41.1% | PF=1.375 | Return=+57.95% | MaxDD=10.76%

Parameters:
  ATR_PERIOD_15M   = 14       (bars on 15m timeframe)
  ATR_SL_MULT      = 1.0      (minimum SL = 1.0 × ATR14 on 15m)

Change scope: SL/TP calculation only.  All entry filters identical to EXP009.

Decision criteria:
  KEEP if:
    - PF maintained or improves on both assets
    - MaxDD does not increase materially (>1pp)
    - Win rate improves (fewer SL hits from noise)
    - Expectancy improves
  REVERT if PF drops or MaxDD worsens significantly
  CONDITIONAL if improvement is asset-specific
"""

import json
import pandas as pd
import numpy as np

import sys
import os
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from core.strategy_pullback import (
    prepare_1h, prepare_15m, align_1h_to_15m,
    get_trend, is_trend_strong, is_pullback_quality,
    is_entry_trigger, is_candle_quality,
    is_range_sufficient, is_market_efficient,
)
from core.trade_logic import check_exit
from backtest.backtest_v2 import load_data

# ── Config ────────────────────────────────────────────────────────────────────
INITIAL_CAPITAL  = 10_000.0
RISK_PCT         = 0.01
MIN_RISK_PRICE   = 1.0
MIN_SL_DIST_PCT  = 0.0015       # EXP007 floor (kept for compatibility)

# ── EXP011 parameters ─────────────────────────────────────────────────────────
ATR_PERIOD_15M  = 14
ATR_SL_MULT     = 1.0           # minimum SL = 1.0 × ATR14 on 15m


def atr(df: pd.DataFrame, period: int = 14) -> pd.Series:
    """True Range and ATR on OHLC dataframe."""
    hi = df['high']
    lo = df['low']
    pc = df['close'].shift(1)
    tr = pd.concat([hi - lo, (hi - pc).abs(), (lo - pc).abs()], axis=1).max(axis=1)
    return tr.ewm(span=period, adjust=False).mean()


def calculate_sl_tp_atr(
    entry: float,
    direction: str,
    candle_low: float,
    candle_high: float,
    atr_val: float,
) -> tuple[float, float] | tuple[None, None]:
    """
    SL = max(candle extreme distance, ATR_SL_MULT × ATR14_15m).
    TP = 2× actual risk used (R:R 2:1 maintained).
    """
    if direction == 'long':
        candle_risk = entry - candle_low
        min_risk    = ATR_SL_MULT * atr_val
        risk        = max(candle_risk, min_risk)
        if risk <= MIN_RISK_PRICE:
            return None, None
        sl = entry - risk
        tp = entry + 2.0 * risk
    else:
        candle_risk = candle_high - entry
        min_risk    = ATR_SL_MULT * atr_val
        risk        = max(candle_risk, min_risk)
        if risk <= MIN_RISK_PRICE:
            return None, None
        sl = entry + risk
        tp = entry - 2.0 * risk

    return sl, tp


def run(path_1h: str, path_15m: str, asset: str, longs_only: bool) -> dict:
    df_1h_raw, df_15m_raw = load_data(path_1h, path_15m)

    df_1h  = prepare_1h(df_1h_raw)
    df_15m = prepare_15m(df_15m_raw)

    # ATR on 15m
    df_15m['atr14'] = atr(df_15m, ATR_PERIOD_15M)

    df = align_1h_to_15m(df_15m, df_1h)

    equity   = INITIAL_CAPITAL
    position = None
    trades   = []
    peak     = equity

    for idx, row in df.iterrows():
        if pd.isna(row.get('ema20')) or pd.isna(row.get('er24')):
            continue

        # ── Manage open position ──────────────────────────────────────────────
        if position is not None:
            result = check_exit(position['direction'], position['sl'], position['tp'], row)
            if result is not None:
                reason, exit_price = result
                risk_used = position['risk_used']
                if reason == 'TP':
                    pnl = 2.0 * risk_used
                else:
                    pnl = -risk_used
                equity += pnl
                peak = max(peak, equity)
                trades.append({
                    'open_time':  position['open_time'],
                    'close_time': row['open_time'],
                    'direction':  position['direction'],
                    'entry':      position['entry'],
                    'exit':       exit_price,
                    'sl':         position['sl'],
                    'tp':         position['tp'],
                    'reason':     reason,
                    'pnl':        round(pnl, 4),
                    'equity':     round(equity, 4),
                    'atr_at_entry': position['atr_at_entry'],
                    'candle_risk':  position['candle_risk'],
                    'atr_risk':     position['atr_risk'],
                })
                position = None

        if position is not None:
            continue

        # ── Entry logic (identical to EXP009) ────────────────────────────────
        trend = get_trend(row)
        if trend is None:
            continue
        if longs_only and trend != 'up':
            continue
        if not is_trend_strong(row):
            continue
        if not is_pullback_quality(row, trend):
            continue
        if not is_entry_trigger(row, trend):
            continue
        if not is_candle_quality(row, trend):
            continue
        if not is_range_sufficient(row):
            continue
        if not is_market_efficient(row):
            continue

        # ── SL/TP with ATR floor ──────────────────────────────────────────────
        atr_val = row.get('atr14')
        if pd.isna(atr_val) or atr_val <= 0:
            continue

        entry = row['close']
        sl, tp = calculate_sl_tp_atr(
            entry, trend, row['low'], row['high'], atr_val
        )
        if sl is None:
            continue

        # Minimum SL distance as % (EXP007 floor)
        if abs(entry - sl) / entry < MIN_SL_DIST_PCT:
            continue

        risk_used = equity * RISK_PCT

        # Track how ATR changed the SL vs the candle extreme
        if trend == 'long':
            candle_risk = entry - row['low']
        else:
            candle_risk = row['high'] - entry
        atr_risk = ATR_SL_MULT * atr_val

        position = {
            'open_time':    row['open_time'],
            'direction':    trend,
            'entry':        entry,
            'sl':           sl,
            'tp':           tp,
            'risk_used':    risk_used,
            'atr_at_entry': atr_val,
            'candle_risk':  candle_risk,
            'atr_risk':     atr_risk,
        }

    return {'trades': trades, 'final_equity': equity}


def metrics(trades: list) -> dict:
    if not trades:
        return {}
    df = pd.DataFrame(trades)
    wins  = df[df['pnl'] > 0]
    loss  = df[df['pnl'] <= 0]
    gross_profit = wins['pnl'].sum()
    gross_loss   = abs(loss['pnl'].sum())
    pf = gross_profit / gross_loss if gross_loss > 0 else float('inf')
    peak = INITIAL_CAPITAL
    max_dd = 0.0
    eq = INITIAL_CAPITAL
    for pnl in df['pnl']:
        eq += pnl
        peak = max(peak, eq)
        dd = (peak - eq) / peak * 100
        max_dd = max(max_dd, dd)
    total_ret = (df['equity'].iloc[-1] - INITIAL_CAPITAL) / INITIAL_CAPITAL * 100
    return {
        'total_trades':   len(df),
        'win_rate_pct':   round(len(wins) / len(df) * 100, 1),
        'return_pct':     round(total_ret, 2),
        'max_dd_pct':     round(max_dd, 2),
        'profit_factor':  round(pf, 3),
        'expectancy_usd': round(df['pnl'].mean(), 2),
        'atr_expanded_pct': round((df['atr_risk'] > df['candle_risk']).mean() * 100, 1),
    }


def monthly(trades: list) -> list:
    if not trades:
        return []
    df = pd.DataFrame(trades)
    df['month'] = pd.to_datetime(df['open_time']).dt.to_period('M')
    rows = []
    for month, g in df.groupby('month'):
        wins = g[g['pnl'] > 0]
        gl   = abs(g[g['pnl'] <= 0]['pnl'].sum())
        gp   = wins['pnl'].sum() if len(wins) else 0.0
        pf   = gp / gl if gl > 0 else float('inf')
        rows.append({
            'month': str(month), 'trades': len(g),
            'win_rate_pct': round(len(wins) / len(g) * 100, 1),
            'net_pnl': round(g['pnl'].sum(), 2),
            'profit_factor': round(pf, 3),
        })
    return rows


BASELINE = {
    'BTC': {
        'total_trades': 99, 'win_rate_pct': 39.4, 'return_pct': 18.45,
        'max_dd_pct': 7.78, 'profit_factor': 1.297, 'expectancy_usd': 18.63,
        'monthly': {
            '2025-09': -100.00, '2025-10': 278.74, '2025-11': -697.14,
            '2025-12': 272.77,  '2026-01': 999.21, '2026-02': 92.45, '2026-03': 998.52,
        },
    },
    'ETH': {
        'total_trades': 207, 'win_rate_pct': 41.1, 'return_pct': 57.95,
        'max_dd_pct': 10.76, 'profit_factor': 1.375, 'expectancy_usd': 27.99,
        'monthly': {
            '2025-09': -299.90, '2025-10': 1971.19, '2025-11': 561.32,
            '2025-12': -279.11, '2026-01': 814.04,  '2026-02': 2459.05, '2026-03': 568.01,
        },
    },
}


def print_report(asset: str, m: dict, mo: list) -> None:
    b = BASELINE[asset]
    print(f"\n{'='*70}")
    print(f"EXP011  |  {asset}  |  ATR-based minimum SL (ATR14×{ATR_SL_MULT})")
    print(f"{'='*70}")

    print(f"\n{'Metric':<25} {'EXP009 base':>13} {'EXP011':>13} {'Delta':>10}")
    print(f"{'-'*63}")
    fields = [
        ('Trades',        'total_trades',   '{:d}'),
        ('Win rate %',    'win_rate_pct',    '{:.1f}'),
        ('Return %',      'return_pct',      '{:+.2f}'),
        ('Max DD %',      'max_dd_pct',      '{:.2f}'),
        ('Profit factor', 'profit_factor',   '{:.3f}'),
        ('Expectancy $',  'expectancy_usd',  '{:+.2f}'),
    ]
    for label, key, fmt in fields:
        bv    = b[key]
        new   = m.get(key, 0)
        delta = new - bv
        sign  = '+' if delta >= 0 else ''
        print(f"  {label:<23} {fmt.format(bv):>13} {fmt.format(new):>13}  {sign}{fmt.format(delta):>8}")

    atr_exp = m.get('atr_expanded_pct', 0)
    print(f"\n  SL expanded by ATR : {atr_exp:.1f}% of trades (ATR was wider than candle extreme)")

    print(f"\n── Monthly breakdown ────────────────────────────────────────────────")
    print(f"  {'Month':<10} {'Trades':>6} {'WR':>6} {'PnL':>10} {'PF':>7}  {'vs EXP009':>10}")
    print(f"  {'-'*60}")
    for row in mo:
        pf_str = f"{row['profit_factor']:.3f}" if row['profit_factor'] != float('inf') else "  inf"
        flag   = '  *' if row['profit_factor'] < 1.0 else ''
        base_pnl = b['monthly'].get(row['month'], 0.0)
        delta_pnl = row['net_pnl'] - base_pnl
        print(f"  {row['month']:<10} {row['trades']:>6} {row['win_rate_pct']:>5.1f}% {row['net_pnl']:>+9.2f}  {pf_str}{flag}  {delta_pnl:>+9.2f}")
    print(f"{'='*70}\n")


if __name__ == '__main__':
    configs = [
        {'name': 'BTC', 'path_1h': 'data/BTCUSDT_1h_last_200d.csv',
         'path_15m': 'data/BTCUSDT_15m_last_180d.csv', 'longs_only': True},
        {'name': 'ETH', 'path_1h': 'data/ETHUSDT_1h_last_200d.csv',
         'path_15m': 'data/ETHUSDT_15m_last_180d.csv', 'longs_only': False},
    ]

    all_results = {}
    for cfg in configs:
        print(f"\nRunning {cfg['name']}...")
        result = run(cfg['path_1h'], cfg['path_15m'], cfg['name'], cfg['longs_only'])
        m  = metrics(result['trades'])
        mo = monthly(result['trades'])
        print_report(cfg['name'], m, mo)
        all_results[cfg['name']] = {'metrics': m, 'monthly': mo,
                                    'final_equity': result['final_equity']}

    out_path = 'data/backtest_exp011.json'
    with open(out_path, 'w') as f:
        json.dump(all_results, f, indent=2)
    print(f"Results saved to {out_path}")
