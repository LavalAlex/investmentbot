"""
TASK 010 — 4h trend alignment filter

Base: COMBINED final (SLOPE_CAP + DYN-B + TIME-B + ETH-SHORT-C) from Task007.

Hypothesis:
    The system uses 1h for trend context and 15m for entry trigger. Adding a
    higher timeframe (4h) as a macro direction filter should reduce counter-trend
    entries — trades that fire on a 1h pullback but are fighting the 4h trend.

    A trade aligned with 3 timeframes (4h + 1h + 15m) should have a materially
    higher win rate than one aligned with only 2 (1h + 15m).

    4h EMA20 is computed by resampling the 1h data (no extra download needed).

Variants:
    BASE        : current system (no 4h filter)
    ALIGN_4H_A  : only trade in direction of 4h EMA20 slope (any positive/negative)
    ALIGN_4H_B  : 4h EMA20 slope must be meaningful (|slope| > 0.03%/5 4h-bars)
                  — avoids flat 4h markets where direction is ambiguous
"""
...
EMA_PERIOD_4H   = 20
SLOPE_LB_4H     = 5     # bars (5 × 4h = 20h lookback for slope)
MIN_SLOPE_4H_B  = 0.03  # %: variant B requires meaningful slope

VARIANTS = {
    'BASE':       {'use_4h': False, 'min_slope': None},
    'ALIGN_4H_A': {'use_4h': True,  'min_slope': 0.0},
    'ALIGN_4H_B': {'use_4h': True,  'min_slope': MIN_SLOPE_4H_B},
}
...
def build_4h_context(df_1h_raw: pd.DataFrame) -> pd.DataFrame:
    """
    Resample 1h OHLCV to 4h and compute EMA20 + slope.
    Returns a df with columns: open_time_4h, ema20_4h, slope_4h_pct, available_at_4h.
    available_at_4h = open_time_4h + 4h (when this bar is fully closed and usable).
    """
    df = df_1h_raw.copy()
    ...
    return df4[['available_at_4h', 'ema20_4h', 'slope_4h_pct']].dropna()
...
def align_full(df_15m, df_1h_prep, df_4h_ctx):
    ...
    df  = pd.merge_asof(df, df4, left_on='open_time', right_on='available_at_4h',
                        direction='backward')
    return df
...
def passes_4h_filter(row, direction, vcfg) -> bool:
    if not vcfg['use_4h']:
        return True
    slope_4h = row.get('slope_4h_pct')
    if slope_4h is None or pd.isna(slope_4h):
        return False
    min_slope = vcfg['min_slope']
    if direction == 'long':
        return slope_4h > min_slope
    else:
        return slope_4h < -min_slope
...
def run(df, longs_only, min_sl_pct, vcfg, asset_name, start=None, end=None):
    ...
    for _, row in df.iterrows():
        ...
        # 4h alignment filter
        if not passes_4h_filter(row, direction, vcfg):
            stats['filtered_4h'] += 1
            continue
        ...
        slope_4h = row.get('slope_4h_pct', float('nan'))
        ...
        position = {
            ...
            'slope_4h': round(float(slope_4h), 4) if not pd.isna(slope_4h) else None,
        }
    return trades, stats
...
if __name__ == '__main__':
    print(f"\nTASK010 — 4h trend alignment filter  |  Base: COMBINED (Task007 ETH-SHORT-C)  |  730d\n")
    all_results = {}
    ...
    for asset_name, cfg in ASSET_CONFIGS.items():
        ...
        for vname, vcfg in VARIANTS.items():
            trades, stats = run(df, cfg['longs_only'], cfg['min_sl_pct'], vcfg, asset_name)
            m = metrics(trades)
            asset_results[vname] = {'metrics': m, 'trades': trades, 'stats': stats}
            filtered = stats['filtered_4h']
            print(f"  [{vname:12}] trades={m['total_trades']:>3}  PF={m['profit_factor']:.3f}  "
                  f"WR={m['win_rate_pct']:.1f}%  Return={m['return_pct']:+.1f}%  "
                  f"MaxDD={m['max_dd_pct']:.2f}%"
                  + (f"  filtered_4h={filtered}" if filtered > 0 else ""))
        all_results[asset_name] = asset_results

        # Direction breakdown for ALIGN_4H_A
        if 'ALIGN_4H_A' in asset_results:
            tdf = pd.DataFrame(asset_results['ALIGN_4H_A']['trades'])
            if not tdf.empty:
                print(f"\n  ── PF por dirección (ALIGN_4H_A) {asset_name}:")
                for d in ['long', 'short']:
                    sub = tdf[tdf['direction'] == d]
                    if sub.empty: continue
                    wins_d = sub[sub['pnl'] > 0]
                    gl_d   = abs(sub[sub['pnl'] <= 0]['pnl'].sum())
                    pf_d   = wins_d['pnl'].sum() / gl_d if gl_d > 0 else float('inf')
                    print(f"  {d:<6}  T={len(sub):>3}  WR={len(wins_d)/len(sub)*100:.0f}%  PF={pf_d:.3f}")

        print(f"\n  {'Métrica':<20}" + ''.join(f"  {v:>14}" for v in VARIANTS))
        print(f"  {'-'*70}")
    ...
    print(f"\n{'='*80}")
    print(f"  TASK010  |  Walk-forward combinado BTC+ETH (4×182d)")
    print(f"{'='*80}")
    print(f"  {'Ventana':<6}  {'Período':<22}" + ''.join(f"  {v:>16}" for v in VARIANTS))
    print(f"  {'-'*80}")
    for wid, start, end, label in WINDOWS:
        ...
        for vname in VARIANTS:
            ...
            row_str += f"  {cpf:>13.3f} {flag}"
        print(row_str)

    out_path = 'data/backtest_task010.json'
