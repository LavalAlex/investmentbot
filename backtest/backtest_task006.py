...
Hypothesis:
    DYN-B (dynamic sizing 0.6–1.2×) and TIME-B (07–21 UTC) are orthogonal:
    - TIME-B eliminates bad entry hours (removes trades)
    - DYN-B scales size inversely with volatility (modifies sizing)
    Their effects should accumulate rather than overlap.

Variants:
    BASE     : SLOPE_CAP only (EXP019 reference)
    DYN-B    : SLOPE_CAP + dynamic sizing 0.6–1.2×
    TIME-B   : SLOPE_CAP + hours 07–21 UTC
    COMBINED : SLOPE_CAP + DYN-B + TIME-B  (full system)
"""
...
# TIME-B window
TIME_B_START = 7   # inclusive
TIME_B_END   = 21  # exclusive

VARIANTS = {
    'BASE':     {'use_dyn': False, 'use_time': False},
    'DYN-B':    {'use_dyn': True,  'use_time': False},
    'TIME-B':   {'use_dyn': False, 'use_time': True},
    'COMBINED': {'use_dyn': True,  'use_time': True},
}
...
def run(df, longs_only, min_sl_pct, vcfg, start=None, end=None):
    ...
    for _, row in df.iterrows():
        ...
        if vcfg['use_time'] and not (TIME_B_START <= hour < TIME_B_END):
            continue
        ...
        scale     = get_scale(atr_ratio_val) if vcfg['use_dyn'] else 1.0
...
def metrics(trades):
    ...
    monthly = df.groupby(df['close_time'].str[:7])['pnl'].sum() / INITIAL_CAPITAL * 100
...
if __name__ == '__main__':
    print(f"\nTASK006 — DYN-B + TIME-B  |  SLOPE_CAP=0.20%  |  730d con fees\n")
    all_results = {}
    ...
    for asset_name, cfg in ASSET_CONFIGS.items():
        ...
        for vname, vcfg in VARIANTS.items():
            ...
            print(f"  [{vname:8}] trades={m['total_trades']:>3}  PF={m['profit_factor']:.3f}  "
                  f"WR={m['win_rate_pct']:.1f}%  Return={m['return_pct']:+.1f}%  "
                  f"MaxDD={m['max_dd_pct']:.2f}%  Sharpe={m['sharpe']:.2f}")
    ...
    # Walk-forward
    print(f"\n{'='*80}")
    print(f"  TASK006  |  Walk-forward combinado BTC+ETH (4×182d)")
    print(f"{'='*80}")
    ...
    out_path = 'data/backtest_task006.json'
